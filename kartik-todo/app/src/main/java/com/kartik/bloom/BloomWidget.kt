    ...
    companion object {
        ...
        private fun update(context: Context, manager: AppWidgetManager, id: Int) {
            ...
            val tasks = loadTasks(context).filter { !it.done && it.due <= today }
                .sortedWith(compareByDescending<BloomTask> { smartTaskScore(it) }.thenBy { it.time })
            ...
            views.setTextViewText(R.id.widget_task, next?.let { "${formatTaskTime(it.time)}  ${it.title} · ${it.priority.label}" } ?: "All clear ✨")
